...
async def aboutd5ev(client: Client, message: Message):
    usr = await client.get_chat(6570358533)
    name = usr.first_name
    bio = (await client.get_chat(6570358533)).bio
    async for photo in client.iter_profile_photos(6570358533, limit=1):
                    await message.reply_photo(photo.file_id, caption=f"""- 𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓 𝑻𝒐 𝑩𝒐𝒕 𝑴𝒊𝒓𝒂 ♪ -› @si_ero\n\n- 𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓'𝒔 𝑩𝒊𝒐 -› {bio}""", 
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
